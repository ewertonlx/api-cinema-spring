...
public class MovieService {
    ...
    private MovieDTO getMovieResponse(Movie movie) {
        return new MovieDTO(
                ...
                movie.getDirector(),
                movie.getCategories(),
                movie.getFeedbacks().stream().map(f -> new FeedBackDTO(f.getUsername(), f.getComment(), f.getRating())).toList()
        );
    }

    public MovieDTO getMovieById(int id){
        return repository.findById(id)
        .map(movie -> new MovieDTO(movie.getName(), movie.getDescription(), movie.getYear(), movie.getDirector(), movie.getCategories(), movie.getFeedbacks().stream()
        .map(feedback -> new FeedBackDTO(feedback.getUsername(), feedback.getComment(), feedback.getRating())).toList())).orElseThrow(() -> new NotFoundException("Filme não encontrado!"));
    }
    ...
    public List<MovieDTO> getMoviesByCategory(MovieCategory category) {
        ...
        var movies = repository.findAll().stream().filter(movie -> movie.getCategories().contains(category))
            .map(movie -> new MovieDTO(
                ...
                movie.getDirector(),
                movie.getCategories(),
                movie.getFeedbacks().stream().map(feedback -> new FeedBackDTO(feedback.getUsername(), feedback.getComment(), feedback.getRating())).toList())).toList();
        return movies;
    }

    public void updateMovie(int id, MovieDTO movie){
        var movieEntity = repository.findById(id).orElseThrow(() -> new NotFoundException("Filme não encontrado!"));
        if(movie.name() != null){
    ...
    public void deleteMovie(int id){
        Movie movieEntity = repository.findById(id).orElseThrow(() -> new NotFoundException("Filme não encontrado!"));
